...
#+name: confpkg
#+begin_src elisp :var name="" needs="" after="" pre="" prefix="config-" via="copy" emacs-minimum="29.1" :results raw replace :noweb no-export
              ...
              (cond
               ((eq 'copy (plist-get confpkg :via))
                ...
                (narrow-to-region
                 ...
                 (progn
                   (goto-char (point-max))
                   (let ((package (regexp-quote (plist-get confpkg :package))))
                     (re-search-backward
                      (format "[^\n\t ][\n\t ]*\n[\t ]*(provide[ \t\n]+\\(?:'%s\\|(quote[ \t\n]+%s)\\))"
                              package package)))
                   (match-end 0))))
...
;; Doom declares this package before older local recipe repos know about it.
(package! let-completion :recipe (:host github :repo "gggion/let-completion.el"))
;; This upstream repository moved from master to main, but older checkouts keep a
;; master-only fetchspec that breaks `doom sync -u`.
(package! reformatter :recipe (:host github :repo "purcell/emacs-reformatter"
                                :branch "main"))
(unpin! yasnippet)
(unpin! citar)
...
;; (face-remap-add-relative 'solaire-default-face 'test-dirvish)
...
(defun vr/doom-tangle-confpkg-config-h ()
  "Tangle this confpkg literate config before Doom's narrower tangle hook."
  (unless (getenv "__NOTANGLE")
    (let* ((config-file (expand-file-name "config.org" doom-user-dir))
           (default-directory doom-user-dir))
      (when (file-exists-p config-file)
        (require 'compile nil t)
        (unless (and (require 'org nil t)
                     (require 'ob-tangle nil t))
          (user-error "Unable to load Org tangle support"))
        (let ((org-startup-indented nil)
          ...
          (message "Tangling confpkg literate config...")
          (let ((files (org-babel-tangle-file config-file)))
            (unless files
              (user-error "Failed to tangle any blocks from %s" config-file))
            (setenv "__NOTANGLE" "1")
            (message "Tangled confpkg literate config (%d files)" (length files))))))))

(add-hook 'doom-before-sync-hook #'vr/doom-tangle-confpkg-config-h -100)

(with-eval-after-load 'consult
  ;; Doom's Vertico module still customizes the older public source names, while
  ;; current Consult exposes these buffer sources under internal `consult--...`
  ;; names.  Keep the old names bound before Doom runs `consult-customize`.
  (defvar consult-source-bookmark consult--source-bookmark)
  (defvar consult-source-recent-file consult--source-recent-file)
  (defvar consult-source-project-recent-file consult--source-project-recent-file))
...
(setq doom-font (font-spec :family "JetBrainsMono Nerd Font" :size 14)
      ...
      doom-emoji-font (font-spec :family "Twemoji" :size 15)
      doom-serif-font (font-spec :family "Libertinus Serif" :size 15))
...
The ~doom-one~ theme is nice and all, but I want this configuration to stay on a
light theme regardless of the system theme. We still keep the light/dark
preference variables so upstream's adaptive theme helpers have something to
consult, but both sides intentionally resolve to ~doom-one-light~ locally.

#+begin_src emacs-lisp
(defvar doom-theme-light 'doom-one-light
  "The theme to use on light-mode systems.")
(defvar doom-theme-dark 'doom-one-light
  "The theme to use on dark-mode systems.")
#+end_src

First though, let's see if the =DOOM_THEME= environment variable is set, and use
that for explicit temporary overrides. Otherwise, local preference wins and the
startup theme stays light.

#+begin_src emacs-lisp
(setq doom-theme ; Set according to the env var or local light default
    ...
    (when (and (not (display-graphic-p (selected-frame)))
      ...
      (when term-background-rgb
        (setq term-shade (if (< (apply #'+ term-background-rgb) (* 0.6 3 65535))
                             'dark 'light))
        (pcase term-shade
          ('dark (setq doom-theme 'doom-one-light))
...
#+begin_src emacs-lisp :tangle packages.el
;;(package! copilot :recipe (:host github :repo "copilot-emacs/copilot.el" :files ("dist" "copilot.el" "copilot-balancer.el")))
#+end_src
...
(use-package! gptel
  ...
  ;; (setq gptel-api-key (auth-source-pick-first-password :host "gemini-vr.deno.dev"))
  ...
  ;;         (forward-line 1))))
  ;;   (setq-default gptel-model 'gemini-2.0-pro-exp
  ;;                 gptel-log-level 'info
  ;;                 gptel-api-key (auth-source-pick-first-password :host "gemini-vr.deno.dev")
  ;;                 ;; gptel-backend (gptel-make-ollama "Ollama" :models ollama-models :stream t)))
  ;;                 gptel-backend (gptel-make-openai "gemini-vr" :protocol "https" :host "gemini-vr.deno.dev" :endpoint "/v1/chat/completions"
  ;;                 :models '("gemini-2.5-pro-exp")
  ;;                 ;; :models '("gemini-2.5-pro-thinking-exp")
  ;;                 :stream t
  ;;                 :key gptel-api-key
  ;;                 )
  ;;                 ))
  ...
  ;;   :stream t
  ;;   :key "sk-fdsafs"               ;can be a function that returns the key
...
#+begin_src emacs-lisp :tangle packages.el
(package! headlice :recipe (:host nil :repo "https://code.tecosaur.net/tec/headlice.git"
...
(use-package! headlice
  ...
  (setq headlice-preferred-license 'mpl
        ...
        headlice-user-email "vr@vitalyr.com")
...
#+begin_src emacs-lisp :tangle packages.el
(package! mpris :recipe (:host nil :repo "https://code.tecosaur.net/tec/mpris.el.git"))
#+end_src
...
#+begin_src emacs-lisp :tangle packages.el :noweb-ref none
(package! doom-modeline-media-player :recipe (:host nil :repo "https://code.tecosaur.net/tec/doom-modeline-media-player.git"))
#+end_src
...
#+end_src
*** Dirvish
#+call: confpkg("!Pkg Dirvish")

Some lifesaver key bindings for ~Dirvish~ and ~Meow~. Doom doesn't add these key bindings for non-EVIL users.
...
(after! dirvish
  (map! :map dirvish-mode-map
        ...
        :ng  "F"   #'dirvish-layout-toggle
        :ng  "z"   #'dirvish-history-jump
        :ng  "gh"  #'dirvish-subtree-up
        :ng   "gl"  #'dirvish-subtree-toggle
        :ng  "h"   #'dired-up-directory
        :ng  "l"   #'dired-find-file
        :gm [left]  #'dired-up-directory
        :gm [right] #'dired-find-file
        :mg  "[h"  #'dirvish-history-go-backward
        :mg  "]h"  #'dirvish-history-go-forward
        :mg  "[e"  #'dirvish-emerge-next-group
        :mg  "]e"  #'dirvish-emerge-previous-group
        :ng   "r" #'dirvish-subtree-toggle
        ...
        (:prefix ("y" . "yank")
         :ng "l"   #'dirvish-copy-file-true-path
         :ng "n"   #'dirvish-copy-file-name
         :ng "p"   #'dirvish-copy-file-path
         :ng "r"   #'dirvish-copy-remote-path
         :ng "y"   #'dired-do-copy)
        (:prefix ("s" . "symlinks")
         :ng "s"   #'dirvish-symlink
         :ng "S"   #'dirvish-relative-symlink
         :ng "h"   #'dirvish-hardlink))

  (defun vr-set-font-for-mode ()
  (cond
   ((derived-mode-p 'dired-mode)
    (setq-local face-remapping-alist '((default (:family "JetBrains Mono Nerd Font" :height 60 :size 6) default))))
   ;; ((derived-mode-p 'vterm-mode)
   ;;  (setq-local face-remapping-alist '((default (:family "Kode Mono" :height 160) default))))
   ))

  (defun vr-buffer-face-mode-small ()
   "Sets a fixed width (monospace) font in current buffer"
   (interactive)
   (setq buffer-face-mode-face '(:family "JetBrains Mono Nerd Font" :height 50))
   (buffer-face-mode))

  (defun vr-dired-font-decrease ()
  "Decrease font size by 4 levels in `dired-mode`."
  (interactive)
  ...
  ;; Add the function to `dired-mode-hook`.
  (add-hook 'dired-mode-hook #'vr-dired-font-decrease)

  ;; (add-hook 'dired-mode-hook 'vr-set-font-for-mode)
  ;; (add-hook 'dired-mode-hook 'vr-buffer-face-mode-small)
...
(use-package lsp-mode
  ...
  (lsp-rust-analyzer-discriminants-hints t)
  ;; (lsp-rust-analyzer-expression-adjustment-hints t)
  ...
  (lsp-rust-analyzer-closing-brace-hints-min-lines t)
  )
#+end_src
...
Use Apheleia from Doom's :editor (format +onsave) to format TypeScript/TSX and JavaScript/JSX with Biome.

#+begin_src emacs-lisp :tangle config.el
(after! apheleia
  ;; Apheleia already defines a built-in `biome' formatter which uses
  ;; the `apheleia-npx' helper to prefer project-local installs. We only
  ;; need to remap modes from Prettier to Biome.

  ;; Map TS/TSX modes to Biome
...
Typst is a modern typesetting system. We use =typst-ts-mode= (tree-sitter) for
=.typ= files.
...
(package! tip
  :recipe (:host nil :repo "https://git.sr.ht/~mafty/tip"))
...
(use-package! typst-ts-mode
  :defer t
  :init
  (defun vr/typst-tree-sitter-dir ()
    "Return the Doom-local directory for tree-sitter grammars."
    (expand-file-name "tree-sitter" doom-local-dir))

  (defun vr/typst-grammar-library-file ()
    "Return the Typst tree-sitter grammar shared library if present."
    (car (file-expand-wildcards
          (expand-file-name "libtree-sitter-typst.*" (vr/typst-tree-sitter-dir)))))

  (defun vr/typst-install-tree-sitter-grammar ()
    "Install the Typst tree-sitter grammar into Doom's local directory."
    (interactive)
    (require 'treesit)
    (let ((install-dir (vr/typst-tree-sitter-dir))
          (treesit-language-source-alist (copy-sequence treesit-language-source-alist)))
      (add-to-list 'treesit-language-source-alist
                   '(typst "https://github.com/Ziqi-Yang/tree-sitter-typst"))
      (make-directory install-dir t)
      (treesit-install-language-grammar 'typst install-dir)
      (add-to-list 'treesit-extra-load-path install-dir)
      (require 'typst-ts-mode)
      (setq typst-ts-mode-grammar-location (vr/typst-grammar-library-file))
      (message "Installed Typst tree-sitter grammar into %s" install-dir)))

  (defun vr/typst-mode ()
    "Major mode for Typst files with a graceful fallback.
Uses `typst-ts-mode` when the Typst tree-sitter grammar is available; otherwise
falls back to `text-mode`."
    ...
    (if (treesit-ready-p 'typst)
        (typst-ts-mode)
      (text-mode)
      (message "Typst tree-sitter grammar isn't installed. Run M-x vr/typst-install-tree-sitter-grammar")))

  (add-to-list 'auto-mode-alist '("\\.typ\\'" . vr/typst-mode))

  (after! treesit
    (add-to-list 'treesit-language-source-alist
                 '(typst "https://github.com/Ziqi-Yang/tree-sitter-typst"))
    (add-to-list 'treesit-extra-load-path (vr/typst-tree-sitter-dir)))

  :config
  (setq typst-ts-watch-options "--open")
  (setq typst-ts-mode-grammar-location (or typst-ts-mode-grammar-location
                                           (vr/typst-grammar-library-file)))
  (setq typst-ts-lsp-download-path
        (expand-file-name "lsp/tinymist/tinymist" doom-cache-dir))
  ...
  (defvar vr/typst-watch-compiled-hook nil
    "Hook run when typst watch reports a compilation event.")

  (defadvice! vr/typst-ts--watch-process-filter-run-hook-a (_proc output)
    :after #'typst-ts--watch-process-filter
    (when (string-match-p (rx "compiled") output)
      (run-hooks 'vr/typst-watch-compiled-hook)))

  (defun vr/typst-revert-related-pdfs-h (&rest _)
    "Revert visible PDF buffers that correspond to a Typst source file."
    (walk-windows
     (lambda (window)
       (with-selected-window window
         (when-let ((pdf (buffer-file-name)))
           (when (and (string-match-p (rx ".pdf" eos) pdf)
                      (file-exists-p (concat (file-name-sans-extension pdf) ".typ"))
                      (not (buffer-modified-p))
                      (not (file-remote-p pdf)))
             (revert-buffer nil t)))))
     nil
     'visible))

  (add-hook 'typst-ts-compile-after-compilation-hook #'vr/typst-revert-related-pdfs-h)
  (add-hook 'vr/typst-watch-compiled-hook #'vr/typst-revert-related-pdfs-h)

  (after! dtrt-indent
    (add-to-list 'dtrt-indent-hook-mapping-list
                 '(typst-ts-mode default typst-ts-mode-indent-offset)))
  (after! editorconfig
    (add-to-list 'editorconfig-indentation-alist
                 '(typst-ts-mode typst-ts-mode-indent-offset)))
  (after! lsp-mode
  ...
  (after! smartparens
    (defun vr/typst-sp-in-math-p (_id _action _context)
      "Return non-nil if point is inside a Typst math/formula node."
    ...
    (sp-with-modes 'typst-ts-mode
      ...
      (sp-local-pair "*" "*" :actions '(:add insert wrap autoskip navigate) :when nil
                     :unless (list #'vr/typst-sp-in-math-p
...
(use-package! outline-indent-mode
  :defer t
  :commands (outline-indent-mode)
  :init
  (setq outline-indent-indentation-per-level 2)
  :hook (typst-ts-mode . outline-indent-mode))
...
(use-package! tip
  ...
  :commands (tip-mode
             ...
             tip-open)
  :init
  (defun vr/tip-server-default-basedir ()
    "Return a Doom-local default directory for tip-server-py."
    (expand-file-name "tip-server-py" doom-local-dir))

  (defun vr/tip-ensure (&optional force)
    "Start `tip' server if it looks installed in Doom-local state.

`tip' requires an external python jsonrpc server (=tip-server-py=). This command
expects it under `doom-local-dir` as `tip-server-py/`."
    (interactive "P")
    (require 'tip)
    (setq tip-server-basedir (vr/tip-server-default-basedir))
    (cond
     ((not (file-directory-p tip-server-basedir))
      (user-error "tip-server-py not found at %s (see upstream tip README)" tip-server-basedir))
     ((not (file-exists-p (expand-file-name "main.py" tip-server-basedir)))
      (user-error "tip-server-py main.py not found in %s" tip-server-basedir))
     (t
      (tip-ensure force))))

  :config
  ;; Upstream tip currently stores the server process in an inconsistent variable
  ;; name. Keep tip usable without patching the package itself.
  (defadvice! vr/tip--start-server-store-process-a (orig-fn buf port)
    :around #'tip--start-server
    (let ((proc (funcall orig-fn buf port)))
      (setq tip--server-process proc)
      proc))

  (map! :map typst-ts-mode-map
        :localleader
        (:prefix ("i" . "inline preview")
         :desc "Toggle tip" "i" #'tip-mode
         :desc "Ensure server" "s" #'vr/tip-ensure
         :desc "Render window" "r" #'tip-send-nbd
         :desc "Render buffer" "R" #'tip-send-all
         :desc "Clear all" "c" #'tip-clear-all
         :desc "Open overlay" "o" #'tip-open
         :desc "Update preamble" "p" #'tip-update-preamble
         :desc "Live setup" "l" #'tip-live-setup
         :desc "Live teardown" "L" #'tip-live-teardown)))
...
(use-package! typst-preview
  ...
  (map! :map typst-ts-mode-map
        ...
        (:prefix ("P" . "typst-preview")
         :desc "Toggle mode" "p" #'typst-preview-mode
         :desc "Start" "s" #'typst-preview-start
         :desc "Stop" "S" #'typst-preview-stop
         :desc "Restart" "r" #'typst-preview-restart
         :desc "Send position" "j" #'typst-preview-send-position)))
